...
#HAMILTONIANO#
H1 = D1*(S33A) + D1*S22A - D2*(S22A) + DP*S11A - DP*(a.dag()*a)+g*S13A.dag()*a + g*a.dag()*S13A +E*a+E*a.dag() #Interação 
...
plt.yscale("log")
plt.plot(OList,C, linewidth=2 )
plt.savefig('figure.png', format='png')

#DATA BACKUP
#f = open("1atomo-data", "w")
#f.write("x y\n")        # column names
#np.savetxt(f, np.array([OList, C]).T)
